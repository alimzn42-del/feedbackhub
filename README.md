...
**481 tests** — 247 API, 234 web. None needs a running Keycloak and none needs
a database.
...
carries one of two trailers, and no commit carries neither:
...
that were substantially AI-generated; it is on every commit except eight. Those
eight carry the second, which is the trailer the Claude Code harness appends
itself; they were made through that path between 23 and 25 August — the
filters, settings, translation and approval slices — same meaning, different
spelling. The trailer being on **every** commit is the honest
application of the convention, not a courtesy: every line in this repository
was generated by the assistant under direction, and no commit was written or
rewritten by hand. How that was directed, and where it went wrong, is
...
| `mailpit` | where the realm's email goes | **local review only**: the realm verifies addresses at registration, and this is the SMTP sink the verification link lands in, at <http://localhost:8025>. A deployment names a real relay in the realm instead |
...
link, and the flow continues to the board. Whether the board then admits the
